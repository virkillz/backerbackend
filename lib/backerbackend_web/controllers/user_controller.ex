defmodule BackerbackendWeb.UserController do
  use BackerbackendWeb, :controller

  alias Backerbackend.Logging
  alias Backerbackend.Account
  alias Backerbackend.Account.User
  alias Backerbackend.Auth.Guardian
  ...
  def login(conn, _params) do
      changeset = Account.change_user(%User{})
      render conn, "login.html", layout: {BackerbackendWeb.LayoutView, "center.html"}, changeset: changeset
  end
  ...
  case Account.authenticate_user(user, password) do
    ...
    {:error, reason} ->
      ...
      |> render("login.html", layout: {BackerbackendWeb.LayoutView, "center.html"}, changeset: changeset)
    end
  end
  ...
  def locked(conn, _) do
    ...
    conn
    |> Guardian.logout
    |> render("locked.html", layout: {BackerbackendWeb.LayoutView, "center.html"}, name: name, avatar: avatar, username: username, changeset: changeset)
  end
